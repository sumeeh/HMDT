    ...
    <div class="col-lg-12 col-md-12 col-sm-12">
        ...
        <main id="main" class="main">
            ...
            <div class="row">
                ...
                $nurse_id = isset($_GET['nurse_id']) && is_numeric($_GET['nurse_id']) ? intval($_GET['nurse_id']) : 0;

                $follow_up_id = isset($_GET['follow_up_id']) && is_numeric($_GET['follow_up_id']) ? intval($_GET['follow_up_id']) : 0;

                $stmt = $pdo->prepare("UPDATE follow_up_record SET status = ? , nurse_id = ? WHERE follow_up_record_id = ?");
                
                $stmt->execute(array(1 , $nurse_id , $follow_up_id));
                ...
                echo '<div class="container" dir="ltr" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
                        ...
                        Patient\'s medication has been delivered successfully
                        </div>
                    </div>'; 
            ...
            ?>    
                    ...
                    $stmt = $pdo->prepare("SELECT * FROM nurses WHERE user_id='$id'");
                    $stmt->execute();
                    $rows = $stmt->fetch();
                    $count = $stmt->rowCount();
                    ...
                    $stmte = $pdo->prepare("SELECT * FROM patients WHERE patient_id='$patient_id'");
                    $stmte->execute();
                    $rowsZ = $stmte->fetch();
                    $countt = $stmte->rowCount();
                    ...
                    <div class="col-lg-12" style="border-radius: 10px;background-color: #FFF;border: 1px solid #CCC;padding: 10px;margin-bottom: 5px">
                        <div class="row">
                            ...
                            <div style="font-weight: bold;line-height: 2.2" class="col-lg-9">
                                ...
                                <?php
                                include('../db/db.inc.php');
                                $sqll = $pdo->prepare("SELECT * FROM patient_files WHERE patient_id='$patient_id'");      
                                $sqll->execute();
                                $rowss = $sqll->fetchAll();
                                ...
                                ?>
                                    <li style="background-color: #EAF6F4;border-radius: 5px;margin-bottom: 6px;padding: 5px"><?php if(!empty($infoo['chronic_diseases'])){ echo $infoo['chronic_diseases']; }else{echo "Chronic Diseases Not Found";} ?></li>
                                    <li style="background-color: #EAF6F4;border-radius: 5px;margin-bottom: 6px;padding: 5px"><?php if(!empty($infoo['allergies'])){ echo $infoo['allergies']; }else{echo "Allergies Not Found";} ?></li>
                                    <?php } ?>
                                </ul>
                            </div>
                    ...
                    <div class="col-lg-12" style="border-radius: 10px;background-color: #FFF;border: 1px solid #CCC;padding: 10px;margin-bottom: 5px">
                        ...
                        <?php
                            ...
                            foreach ($rows as $pat) {
                                ...
                                if (strtotime($time_now) < strtotime($meding_time) && $today == $db_date) {
                                    include('../db/db.inc.php');
                                ...
                                <div class="row">
                                    ...
                                    <div style="background-color: #EAF6F4;border-radius: 5px;margin-bottom: 6px;padding: 5px;line-height: 2.2" class="col-lg-9 col-sm-10">
                                        ...
                                        <?php
                                            ...
                                            ?> 
                                            <span style="font-weight: bold;color: #329287">Notes:</span> <?php echo $info['notes'] ?> <br/>
                                            <span style="font-weight: bold;color: #329287">Date:</span> <?php echo $info['notes_date'] ?> <br/>
                                            <span style="font-weight: bold;color: #329287">Time:</span> <?php echo $info['notes_time'] ?> <br/>
                                            <?php } ?>
                                        </ul>
                                    </div>
                                    <div style="font-weight: bold" class="col-lg-1 col-sm-1"></div>
